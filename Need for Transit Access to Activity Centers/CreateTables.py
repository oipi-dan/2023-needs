...
output_dir = r"C:\Users\daniel.fourquet\Documents\Tasks\VTrans Update\Mid-Term Needs\Need for Transit Access to Activity Centers\data"

if not os.path.exists(output_dir):
    raise Exception("Output directory does not exist.")


def build_url(table, data, geography, year):
    URL_base = f"https://api.census.gov/data/{year}/acs/acs5?"
    if table['tableType'] == 'Subject':
        URL_base = f"https://api.census.gov/data/{year}/acs/acs5/subject?"
    ...
    try:
        ...
        if table['functions']:
            for f in table['functions']:
                ...
                if funcOperation == "diff":
                    df[funcName] = abs(df[funcStatistics[0]]-df[funcStatistics[1]])
...
def download_tables(table, year):
    ...
    # Create URL
    URL = build_url(table, data, geography, str(year))
